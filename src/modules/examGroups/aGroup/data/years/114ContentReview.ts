...
function hasContinuousUniqueNumbers(questions: readonly ExamQuestionAnalysis[]): boolean {
  const numbers = new Set(questions.map((question) => question.number));

  return questions.length === 50 && Array.from({ length: 50 }, (_, index) => index + 1).every((number) => numbers.has(number));
}
...
function hasTraceableAnswerState(question: ExamQuestionAnalysis): boolean {
  return (
    question.tags.length > 0 &&
    question.tags.every((tag) => tag.trim().length > 0) &&
    question.sourceRef.year === '114' &&
    question.sourceRef.fileName === '114.pdf' &&
...
export function getAGroup114ContentReviewChecklist(
  ...
  return [
    { label: '50 題題號唯一且連續', passed: hasContinuousUniqueNumbers(questions) },
    { label: '每題皆有 A 至 D 原始選項與四份選項辨析', passed: questions.every(hasAllOptionsAndExplanations) },
    { label: '每題皆有非空 tags、PDF sourceRef 與官方答案狀態', passed: questions.every(hasTraceableAnswerState) },
...
export function validateAGroup114QuestionContent(
  ...
  for (const check of getAGroup114ContentReviewChecklist(questions)) {
    if (!check.passed) {
