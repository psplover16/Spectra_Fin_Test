...
describe('107 A group source extraction baseline', () => {
  it('contains one source row for each 107 question', () => {
  ...
  it('maps every row to original exam content, official answer, and PDF sourceRef', () => {
    for (const row of sourceBaselineRows) {
      ...
      expect(row.sourceRef.year).toBe('107');
      expect(row.sourceRef.fileName).toBe('107.pdf');
