...
describe('B group 108 question content', () => {
  const sourceIndex = getBGroupSourceIndex('108');

  it('contains one analysis for every indexed 108 source question', () => {
    ...
    expect(questions.every((question) => question.year === '108')).toBe(true);
  });
  ...
  it('keeps high-risk 108 extraction questions under review', () => {
    [1, 4, 6].forEach((number) => {
