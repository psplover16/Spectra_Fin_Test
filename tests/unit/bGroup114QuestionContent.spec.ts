...
describe('B group 114 question content', () => {
  const sourceIndex = getBGroupSourceIndex('114');

  it('contains one reviewed analysis for every indexed 114 source question', () => {
    ...
    expect(questions.every((question) => question.year === '114')).toBe(true);
  });
