...
describe('B group 107 content review', () => {
  const config = { year: '107', fileName: '107.pdf' } as const;
  ...
  it('rejects missing circular queue details', () => {
  ...
  it('rejects a source trace with a mismatched original excerpt', () => {
    const wrongSourceQuestion: BGroupEssayQuestionAnalysis = {
      ...questions[0],
      sourceRef: {
        ...questions[0].sourceRef,
        originalExcerpt: '錯誤摘錄'
      }
    };
