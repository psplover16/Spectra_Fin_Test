...
} from '@/modules/examGroups/aGroup/data/years/114SourceBaseline';

describe('114 A group source extraction baseline', () => {
  it('contains one source row for each 114 question', () => {
  ...
  it('maps every row to original exam content, official answer, and PDF sourceRef', () => {
    for (const row of sourceBaselineRows) {
      ...
      expect(row.sourceRef.year).toBe('114');
      expect(row.sourceRef.fileName).toBe('114.pdf');
  ...
  it('publishes an explicit content review checklist', () => {
    expect(getSourceBaselineReviewChecklist(sourceBaselineRows)).toEqual([
      ...
      { label: '每題皆標示 114.pdf PDF sourceRef', passed: true }
    ]);
  });
});
