...
describe('B group 110 question content', () => {
  const sourceIndex = getBGroupSourceIndex('110');

  it('contains one analysis for every indexed 110 source question', () => {
    ...
    expect(questions.every((question) => question.year === '110')).toBe(true);
  });
  ...
  it('keeps risky 110 program and SQL extraction questions under review', () => {
    [2, 4, 5, 6].forEach((number) => {
      expect(questions.find((question) => question.number === number)?.reviewStatus).toBe('needs-review');
    });
  });
